...
class FusionComponent(nn.Module):
    ...
    def __init__(self, in_channels1, in_channels2, out_channels):
        ...
        self.se = SE_Block(3*out_channels, 16)
        ...
        self.conv3_1 = Conv(c1=3 *out_channels, c2=32, k=6, s=2, p=2)
    ...
    def forward(self, x1, x2,x3):
        ...
        x3_masked = torch.mul(self.conv_mask1(x3).repeat(1, 3, 1, 1), x3)
        # x4_masked = torch.mul(self.conv_mask2(x4).repeat(1, 3, 1, 1), x4)


        mask_x1 = self.se_1(self.Silu(self.bn1(self.conv1(x1_masked + x1))))
        mask_x2 = self.se_2(self.Silu(self.bn2(self.conv2(x2_masked + x2))))
        mask_x3 = self.se_3(self.Silu(self.bn3(self.conv3(x3_masked + x3))))

        x = self.se(torch.cat([mask_x1, mask_x2,mask_x3], 1))
...
class Ensemble(nn.ModuleList):
    ...
    def forward(self, x1,x2,x3, augment=False, profile=False, visualize=False):
        return self.fusion_component(x1, x2, x3)
